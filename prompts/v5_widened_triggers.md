# v5 -- v4 with a widened rule-1 trigger list
...
Rules:
1. Only extract a parameter if the excerpt states optionality using one of:
   "may", "might", "can", "should", "optional"/"optionally",
   "if present", "is allowed to"/"are allowed to",
   "is permitted to"/"are permitted to", or
   "implementation-defined"/"implementation-specific". The phrase must be
   stated; do not infer optionality from wording outside this list.
...
4. Follow the naming convention `^[A-Z][A-Z_0-9]*$`, matching existing
   parameters in the repo (e.g. `VLEN`, `PMLEN`).
